...
#TODO: add filler items for dustforce
DEFAULT_ITEM_CLASSIFICATIONS = {
    "Downhill Key": ItemClassification.progression | ItemClassification.useful,  # Items can have multiple classifications.
    "Shaded Grove Key": ItemClassification.progression | ItemClassification.useful,
    "Valley Key": ItemClassification.progression | ItemClassification.useful,
    "Firefly Forest Key": ItemClassification.progression | ItemClassification.useful,
    "Dahlia Key": ItemClassification.progression | ItemClassification.useful,
    "Tunnels Key": ItemClassification.progression | ItemClassification.useful,
    "Fields Key": ItemClassification.progression | ItemClassification.useful,
...
def get_random_filler_item_name(world: DustforceWorld) -> str:
    # Dustforce has an option called "trap_chance".
    ...
    if world.random.randint(0, 99) < world.options.trap_chance:
        #TODO: add traps???
        print("Math Trap is not implemented yet, defaulting to Confetti Cannon.")
    return "Confetti Cannon"


def create_item_with_correct_classification(world: DustforceWorld, name: str) -> DustforceItem:
    ...
    return DustforceItem(name, classification, ITEM_NAME_TO_ID[name], world.player)


# With those two helper functions defined, let's now get to actually creating and submitting our itempool.
def create_all_items(world: DustforceWorld) -> None:
    ...
    itempool: list[Item] = [
        world.create_item("Downhill Key"),
        world.create_item("Shaded Grove Key"),
        world.create_item("Valley Key"),
        world.create_item("Firefly Forest Key"),
        world.create_item("Dahlia Key"),
        world.create_item("Tunnels Key"),
        world.create_item("Fields Key"),
    ]
    ...
    ##if world.options.hammer:
        #TODO: add extra items if we add the options?
    ...
    # Dustforce has two of these: The Confetti Cannon and the Math Trap.
    ...
    #  Starting here, the word "filler" will be used to collectively refer to Dustforce's Confetti Cannon and Math Trap,
    ...
    # But there is an alternative that works even better for most worlds, including Dustforce.
    ...
    # Dustforce is one of these games:
    ...
    #TODO: add pre-collected starting items if we want? Potentially provide keys to start with not sure
