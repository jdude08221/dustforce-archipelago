...
def set_all_rules(world: DustforceWorld) -> None:
...
def set_all_entrance_rules(world: DustforceWorld) -> None:
    # First, we need to actually grab our entrances. Luckily, there is a helper method for this.

    #nexus_to_downhill = world.get_entrance("Main Nexus to Downhill")
    #nexus_to_shaded_grove = world.get_entrance("Main Nexus to Shaded Grove")
    nexus_to_valley = world.get_entrance("Main Nexus to Valley")
    nexus_to_firefly_forest = world.get_entrance("Main Nexus to Firefly Forest")
    #nexus_to_dahlia = world.get_entrance("Main Nexus to Dahlia")
    #nexus_to_tunnels = world.get_entrance("Main Nexus to Tunnels")
    nexus_to_fields = world.get_entrance("Main Nexus to Fields")
    ...
    #can_enter_downhill = Has("Downhill Key")
    #can_enter_shaded_grove = Has("Shaded Grove Key")
    can_enter_valley = HasAny("Firefly Forest Key", "Fields Key")
    can_enter_firefly_forest = HasAny("Valley Key",  "Fields Key")
    #can_enter_dahlia = Has("Dahlia Key")
    #can_enter_tunnels = Has("Tunnels Key")
    can_enter_fields = HasAny("Valley Key", "Firefly Forest Key")
    ...
    #world.set_rule(nexus_to_downhill, can_enter_downhill)
    #world.set_rule(nexus_to_shaded_grove, can_enter_shaded_grove)
    world.set_rule(nexus_to_valley, can_enter_valley)
    world.set_rule(nexus_to_firefly_forest, can_enter_firefly_forest)
    #world.set_rule(nexus_to_dahlia, can_enter_dahlia)
    #world.set_rule(nexus_to_tunnels, can_enter_tunnels)
    world.set_rule(nexus_to_fields, can_enter_fields)

    # Conditions can also depend on event items.
    #TODO: update if there are events
    ...
    #TODO: conditional entrance rules based on player options
    ...
    # Dustforce opts to create its Rule objects locally, but just to show what this would look like,
...
def set_all_location_rules(world: DustforceWorld) -> None:
    print("Setting location rules (not implemented)")
...
def set_completion_condition(world: DustforceWorld) -> None:
    # Finally, we need to set a completion condition for our world, defining what the player needs to win the game.
    # For this, we can use world.set_completion_rule.
    # You can just set a completion condition directly like any other condition, referencing items the player receives:
    world.set_completion_rule(HasAll("Downhill Key", "Shaded Grove Key", "Valley Key", "Firefly Forest Key", "Dahlia Key", "Tunnels Key", "Fields Key"))
...
# If your world exclusively uses Rule Builder rules (like Dustforce), it's worth trying CachedRuleBuilderWorld.
# CachedRuleBuilderWorld is a subclass of World that has a bunch of caching magic to make rules faster.
#TODO: potentially switch to CachedRuleBuilderWorld if there are a lot of rules and generation is slow
# Just have your world class subclass CachedRuleBuilderWorld instead of World:
#   class DustforceWorld(CachedRuleBuilderWorld): ...
...
# If you're wondering, author has checked: Dustforce is too simple to see any benefits, so we'll stick with "World".
